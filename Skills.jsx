        ...
        <div className="glass-panel rounded-2xl p-6 sm:p-8 border border-slate-800">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {skillsData.softSkills.map((soft, idx) => (
              <div
                key={idx}
                className="p-5 rounded-xl bg-slate-900/80 border border-slate-800 flex flex-col justify-between hover:border-cyan-500/30 transition-colors"
              >
...
function SkillCard({ skill, IconComp, category, index }) {
  return (
    <motion.div
      ...
      className="glass-panel glass-panel-hover rounded-xl p-5 border border-slate-800/90 flex items-center gap-4 group cursor-pointer"
    >
      <div className={`p-3 rounded-xl bg-gradient-to-br ${skill.color} text-white shadow-md shrink-0 transition-transform group-hover:scale-110`}>
